    ...
    <div id="page-wrapper">
        <div class="container-fluid">
            ...
            if(isset($_POST["submitBtn"])){
                include('connect.php'); 
                $dyear=$_POST["eyear"];
                $januaryq = $db->prepare( "SELECT sum(amount) from sales where month = 'January'AND year = '$dyear'");
                $januaryq->execute();
                for($i=0; $rows = $januaryq->fetch(); $i++){
                   $janr=$rows['sum(amount)'];
                }
                $februaryq = $db->prepare( "SELECT sum(amount) from sales where month = 'February'AND year = '$dyear' ");
                $februaryq->execute();
                for($i=0; $rows = $februaryq->fetch(); $i++){
                   $febr=$rows['sum(amount)'];
                }
                $marchq = $db->prepare( "SELECT sum(amount) from sales where month = 'March'AND year = '$dyear' ");
                $marchq->execute();
                for($i=0; $rows = $marchq->fetch(); $i++){
                   $marr=$rows['sum(amount)'];
                }
                $aprilq = $db->prepare( "SELECT sum(amount) from sales where month = 'April' AND year = '$dyear'");
                $aprilq->execute();
                for($i=0; $rows = $aprilq->fetch(); $i++){
                   $aprilr=$rows['sum(amount)'];
                }
                $mayq = $db->prepare( "SELECT sum(amount) from sales where month = 'May'AND year = '$dyear' ");
                $mayq->execute();
                for($i=0; $rows = $mayq->fetch(); $i++){
                   $mayr=$rows['sum(amount)'];
                }
                $juneq = $db->prepare( "SELECT sum(amount) from sales where month = 'June' AND year = '$dyear'");
                $juneq->execute();
                for($i=0; $rows = $juneq->fetch(); $i++){
                   $junr=$rows['sum(amount)'];
                }
                $julyq = $db->prepare( "SELECT sum(amount) from sales where month = 'July'AND year = '$dyear' ");
                $julyq->execute();
                for($i=0; $rows = $julyq->fetch(); $i++){
                   $julr=$rows['sum(amount)'];
                }
                $augustq = $db->prepare( "SELECT sum(amount) from sales where month = 'August' AND year = '$dyear'");
                $augustq->execute();
                for($i=0; $rows = $augustq->fetch(); $i++){
                   $augr=$rows['sum(amount)'];
                }
                $septemberq = $db->prepare( "SELECT sum(amount) from sales where month = 'September' AND year = '$dyear'");
                $septemberq->execute();
                for($i=0; $rows = $septemberq->fetch(); $i++){
                   $septr=$rows['sum(amount)'];
                }
                $octoberq = $db->prepare( "SELECT sum(amount) from sales where month = 'October' AND year = '$dyear'");
                $octoberq->execute();
                for($i=0; $rows = $octoberq->fetch(); $i++){
                   $octr=$rows['sum(amount)'];
                }
                $novemberq = $db->prepare( "SELECT sum(amount) from sales where month = 'November' AND year = '$dyear'");
                $novemberq->execute();
                for($i=0; $rows = $novemberq->fetch(); $i++){
                   $novr=$rows['sum(amount)'];
                }
                $decemberq = $db->prepare( "SELECT sum(amount) from sales where month = 'December' AND year = '$dyear'");
                $decemberq->execute();
