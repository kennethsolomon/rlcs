                    ...
                    <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                        ...
                        <tbody>
                            ...
                            if(isset($_POST["submitdate"]))
                            {
                                ...
                                $result = $db->prepare("SELECT * FROM purchases WHERE rmonth = '$cmonth' AND ryear = '$cyear' AND status = 'Received' AND remark = 'PAID' ORDER BY transaction_id");
                                $result->execute();
                                for($i=0; $row = $result->fetch(); $i++){
                                    ...
                                    $result = $db->prepare("SELECT * FROM purchases WHERE rmonth = '$cmonth' AND status = 'Received' AND remark = 'PAID' ORDER BY transaction_id desc");
                                    $result->execute();
                                    for($i=0; $row = $result->fetch(); $i++){
                                        ?>
                                         <tr class="record">
                                    ...
                                    $result = $db->prepare("SELECT * FROM purchases WHERE ryear = '$cmonth' AND status = 'Received'  AND remark = 'PAID' ORDER BY transaction_id desc");
                                    $result->execute();
                                    for($i=0; $row = $result->fetch(); $i++){
                                        ?>
                                         <tr class="record">
                                    ...
                                    $result = $db->prepare("SELECT * FROM purchases WHERE status = 'Received'  AND remark = 'PAID' ORDER BY transaction_id desc");
                                    $result->execute();
                                    for($i=0; $row = $result->fetch(); $i++){
                                        ?>
                                        <tr class="record">
                            ...
                            <?php
                                ...
                                echo'</p></center>';
                                if(!empty($dailydate)){
                                $result = $db->prepare("SELECT * FROM purchases WHERE date_deliver = '$dailydate' AND status = 'Received'  AND remark = 'PAID' ORDER BY transaction_id desc");
                                $result->execute();
                                for($i=0; $row = $result->fetch(); $i++){
                                    ?>
                                    <tr class="record">
                            ...
                            <tr>
                                ...
                                <th colspan="3" style="border-top:1px solid #999999"> 
                                    ...
                                    if(isset($_POST["submitdate"]))
                                    {
                                        ...
                                        $cyear=$_POST["cyear"];
                                        if(!empty($cmonth) && !empty($cyear)){
                                                $results = $db->prepare("SELECT sum(cost) FROM purchases WHERE rmonth = '$cmonth' AND ryear = '$cyear'  AND status = 'Received'  AND remark = 'PAID'");
                                                $results->execute();
                                                ...
                                                $results = $db->prepare("SELECT sum(cost) FROM purchases WHERE rmonth = '$cmonth'  AND status = 'Received'  AND remark = 'PAID'");
                                                $results->execute();
                                                for($i=0; $rows = $results->fetch(); $i++){
                                                $dsdsd=$rows['sum(cost)'];
                                                echo formatMoney($dsdsd, true);
                                                ...
                                                $results = $db->prepare("SELECT sum(cost) FROM purchases WHERE ryear = '$cyear'  AND status = 'Received'  AND remark = 'PAID'");
                                                $results->execute();
                                                for($i=0; $rows = $results->fetch(); $i++){
                                                $dsdsd=$rows['sum(cost)'];
                                                echo formatMoney($dsdsd, true);
                                            ...
                                            $results = $db->prepare("SELECT sum(cost) FROM purchases WHERE status = 'Received'  AND remark = 'PAID'");
                                            $results->execute();
                                            for($i=0; $rows = $results->fetch(); $i++){
                                            $dsdsd=$rows['sum(cost)'];
                                            echo formatMoney($dsdsd, true);
                                ...
                                if(isset($_POST["accountduedate"]))
                                {
                                    $dailydate = $_POST['dailydate'];
                                    if(!empty($dailydate)){
                                        $results = $db->prepare("SELECT sum(cost) FROM purchases WHERE date_deliver = '$dailydate' AND status = 'Received' AND remark = 'PAID'");
                                        $results->execute();
                                        for($i=0; $rows = $results->fetch(); $i++){
                                        $dsdsd=$rows['sum(cost)'];
                                        echo formatMoney($dsdsd, true);
                        ...
                        <!-- <thead>
                            ...
                            <th colspan="5" style="border-top:1px solid #999999" >Total balance </th>
                                ...
                                if(isset($_POST["accountduedate"]))
                                {
                                    $dailydate = $_POST['dailydate'];
                                    if(!empty($dailydate)){
                                        $results = $db->prepare("SELECT sum(balance) FROM sales WHERE date = '$dailydate'");
                                        $results->execute();
                                        for($i=0; $rows = $results->fetch(); $i++){
                                        $dsdsd=$rows['sum(balance)'];
                                        echo formatMoney($dsdsd, true);
