        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <form method="post" action="">
                   ...
                   <input style="width: 100px; "name="dailydate" id="dailydate" class="datepicker" placeholder="yyyy-mm-dd" data-date-format="mm-dd-yyyy">
                    ...
                    <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                        ...
                        <tbody>
                            ...
                            if(isset($_POST["submitdate"]))
                            {
                                ...
                                $result = $db->prepare("SELECT * FROM purchases WHERE rmonth = '$cmonth' AND ryear = '$cyear' AND status = 'Received' AND remark = 'PAYABLE' ORDER BY transaction_id");
                                $result->execute();
                                for($i=0; $row = $result->fetch(); $i++){
                                    ...
                                    $result = $db->prepare("SELECT * FROM purchases WHERE rmonth = '$cmonth' AND status = 'Received' AND remark = 'PAYABLE' ORDER BY transaction_id desc");
                                    $result->execute();
                                    for($i=0; $row = $result->fetch(); $i++){
                                        ?>
                                         <tr class="record">
                                    ...
                                    $result = $db->prepare("SELECT * FROM purchases WHERE ryear = '$cyear' AND status = 'Received'  AND remark = 'PAYABLE' ORDER BY transaction_id desc");
                                    $result->execute();
                                    for($i=0; $row = $result->fetch(); $i++){
                                        ?>
                                         <tr class="record">
                                    ...
                                    $result = $db->prepare("SELECT * FROM purchases WHERE status = 'Received'  AND remark = 'PAYABLE' ORDER BY transaction_id desc");
                                    $result->execute();
                                    for($i=0; $row = $result->fetch(); $i++){
                                        ?>
                                        <tr class="record">
                                            <td>PY-000<?php echo $row['transaction_id']; ?></td>
                                            <td><?php echo $row['date_order']; ?></td>
                                            <td><?php echo $row['suplier']; ?></td>
                                            <td><?php echo $row['date_deliver']; ?></td>
                                            <td><?php
                            ...
                            <?php
                                ...
                                echo'</p></center>';
                                if(!empty($dailydate)){
                                $result = $db->prepare("SELECT * FROM purchases WHERE date_order = '$dailydate' AND status = 'Received'  AND remark = 'PAYABLE' ORDER BY transaction_id desc");
                                $result->execute();
                                for($i=0; $row = $result->fetch(); $i++){
                                    ?>
                                    <tr class="record">
                        ...
                        <thead>
                            <tr>
                                <th colspan="6" style="border-top:1px solid #999999"> Total Cost </th>
                                <th colspan="3" style="border-top:1px solid #999999"> 
                                    ...
                                    if(isset($_POST["submitdate"]))
                                    {
                                        ...
                                        $cyear=$_POST["cyear"];
                                        if(!empty($cmonth) && !empty($cyear)){
                                                $results = $db->prepare("SELECT sum(cost) FROM purchases WHERE rmonth = '$cmonth' AND ryear = '$cyear'  AND status = 'Received'  AND remark = 'PAYABLE'");
                                                $results->execute();
                                                ...
                                                $results = $db->prepare("SELECT sum(cost) FROM purchases WHERE rmonth = '$cmonth'  AND status = 'Received'  AND remark = 'PAYABLE'");
                                                $results->execute();
                                                for($i=0; $rows = $results->fetch(); $i++){
                                                $dsdsd=$rows['sum(cost)'];
                                                echo formatMoney($dsdsd, true);
                                                ...
                                                $results = $db->prepare("SELECT sum(cost) FROM purchases WHERE ryear = '$cyear'  AND status = 'Received'  AND remark = 'PAYABLE'");
                                                $results->execute();
                                                for($i=0; $rows = $results->fetch(); $i++){
                                                $dsdsd=$rows['sum(cost)'];
                                                echo formatMoney($dsdsd, true);
                                            ...
                                            $results = $db->prepare("SELECT sum(cost) FROM purchases WHERE status = 'Received'  AND remark = 'PAYABLE'");
                                            $results->execute();
                                            for($i=0; $rows = $results->fetch(); $i++){
                                            $dsdsd=$rows['sum(cost)'];
                                            echo formatMoney($dsdsd, true);
                                ...
                                if(isset($_POST["accountduedate"]))
                                {
                                    $dailydate = $_POST['dailydate'];
                                    if(!empty($dailydate)){
                                        $results = $db->prepare("SELECT sum(cost) FROM purchases WHERE date_order = '$dailydate' AND status = 'Received' AND remark = 'PAYABLE'");
                                        $results->execute();
                                        for($i=0; $rows = $results->fetch(); $i++){
                                        $dsdsd=$rows['sum(cost)'];
                                        echo formatMoney($dsdsd, true);
                            ...
                            <th colspan="5" style="border-top:1px solid #999999" >Total cost </th>
                                  <th colspan="4" style="border-top:1px solid #999999"> 
                                    ...
                                    if(isset($_POST["submitdate"]))
                                    {
                                        ...
                                        $cyear=$_POST["cyear"];
                                        if(!empty($cmonth) && !empty($cyear)){
                                                $results = $db->prepare("SELECT sum(cost) FROM purchases WHERE rmonth = '$cmonth' AND ryear = '$cyear' ");
                                                $results->execute();
                                                for($i=0; $rows = $results->fetch(); $i++){
                                                $dsdsd=$rows['sum(cost)'];
                                                echo formatMoney($dsdsd, true);
                                            ...
                                            $results = $db->prepare("SELECT sum(cost) FROM purchases WHERE rmonth = '$cmonth' ");
                                                $results->execute();
                                                for($i=0; $rows = $results->fetch(); $i++){
                                                $dsdsd=$rows['sum(cost)'];
                                                echo formatMoney($dsdsd, true);
                                            ...
                                            $results = $db->prepare("SELECT sum(cost) FROM purchases WHERE ryear = '$cyear' ");
                                                $results->execute();
                                                for($i=0; $rows = $results->fetch(); $i++){
                                                $dsdsd=$rows['sum(cost)'];
                                                echo formatMoney($dsdsd, true);
                                            ...
                                            $results = $db->prepare("SELECT sum(cost) FROM purchases");
                                            $results->execute();
                                            for($i=0; $rows = $results->fetch(); $i++){
                                            $dsdsd=$rows['sum(cost)'];
                                            echo formatMoney($dsdsd, true);
                                ...
                                if(isset($_POST["accountduedate"]))
                                {
                                    $dailydate = $_POST['dailydate'];
                                    if(!empty($dailydate)){
                                        $results = $db->prepare("SELECT sum(cost) FROM purchases WHERE date = '$dailydate'");
                                        $results->execute();
                                        for($i=0; $rows = $results->fetch(); $i++){
                                        $dsdsd=$rows['sum(cost)'];
                                        echo formatMoney($dsdsd, true);
